...
import type { Interval, QueryWindow } from "@/lib/edge/query/core";
import { resolvePrivateTeamForSession } from "@/lib/edge/query/core";
import type { D1ReadDiagnostics } from "@/lib/edge/query/diagnostics";
import {
...
export async function resolveDashboardSession(
  ...
  if (isDemoBuild) {
    const { resolveDemoDashboardSession } =
      await import("./team-dashboard-demo");
...
export async function readTeamDashboard(
  ...
  if (isDemoBuild) {
    const { readDemoTeamDashboard } = await import("./team-dashboard-demo");
