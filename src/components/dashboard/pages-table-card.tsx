...
function ChangeValue({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-baseline justify-end gap-1 whitespace-nowrap font-mono text-[11px] tabular-nums leading-none",
        changeClass(value, lowerIsBetter),
      )}
    >
      <Icon className="relative top-px size-2.5 shrink-0" />
      {formatChange(value)}
    </span>
  );
}
  ...
  const renderRow = useCallback(
    (row: PagesDashboardRow) => {
      ...
      const renderColumn = (columnId: PagesTableColumnId) => {
          ...
          return (
            <TableCell key="page" className="pl-2">
              <AnalyticsDetailsTooltipTarget
                ...
                locale={locale}
                request={{
                  key: `page-path:${row.pathname}`,
                  items: [
                    {
                      label: messages.common.path,
                      value: decodeUrlDisplayValue(row.pathname || "/"),
        ...
        const change = row.change?.[metric]?.relative ?? null;
        return (
          <TableCell
            key={columnId}
          ...
          >
            {comparisonActive ? (
              <span className="inline-flex w-full items-baseline justify-end gap-1 whitespace-nowrap leading-none">
